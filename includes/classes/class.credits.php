...
class CreditSystem
{
	...
	private $_configCreditsBonusCol;
	...
	protected $webdb;
	protected $accountdb;
	protected $characterdb;
	protected $cabalcashdb;
	protected $common;
	protected $character;
	...
	function __construct()
	{
		...
		// load databases
		$this->webdb = Connection::Database('CabalEngine');
		$this->accountdb = Connection::Database('Account');
		$this->characterdb = Connection::Database('Server01');
		$this->cabalcashdb = Connection::Database('CabalCash');
	 ...
	 * @throws Exception
	 */
	public function addCredits($input, $inputbonus)
	{
		if (!Validator::UnsignedNumber($input)) throw new Exception(lang('error_72'));
		if (!Validator::UnsignedNumber($inputbonus)) throw new Exception(lang('error_72'));
		if (!$this->_configId) throw new Exception(lang('error_66'));
		...
		$data = array(
			...
			'credits' => $input,
			'creditsbonus' => $inputbonus,
			'itemcd' => 301040005,
			'trannumber' => (int) date('YmdHis'),
		);
		$query = "EXEC up_ChargeCash :identifier,  :credits, :creditsbonus, :itemcd, :trannumber";

		// add credits
		$addCredits = $this->cabalcashdb->query($query, $data);
		if (!$addCredits) throw new Exception(lang('error_74'));

		$this->_addLog($config['config_title'], $input, $inputbonus, "Charge");
	}
	 ...
	 * @throws Exception
	 */
	public function substractCredits($input)
	{
		if (!Validator::UnsignedNumber($input)) throw new Exception(lang('error_72'));
		if (!$this->_configId) throw new Exception(lang('error_66'));
		if (!$this->_identifier) throw new Exception(lang('error_73'));
		...
		$query = "EXEC up_RefundCash :identifier,  :refundcredits";

		// add credits
		$refundCredits = $this->cabalcashdb->query($query, $data);
		if (!$refundCredits) throw new Exception(lang('error_74'));

		$this->_addLog($config['config_title'], $input, 0, "Refund");
	}
	...
	public function setConfigCreditsBonusColumn($input)
	{
		if (!Validator::Chars($input, array('a-z', 'A-Z', '0-9', '_'))) throw new Exception(lang('error_81'));
		$this->_configCreditsBonusCol = $input;
	}
	...
	{
		$check = $this->webdb->query_fetch_single("SELECT * FROM " . CABALENGINE_CREDITS_CONFIG . " WHERE config_id = ?", array($input));
	...
	public function saveConfig()
	{
		...
		if (!$this->_configCreditsBonusCol) throw new Exception(lang('error_84'));
		...
		$data = array(
			...
			'creditsbonuscol' => $this->_configCreditsBonusCol,
		...
		$query = "INSERT INTO " . CABALENGINE_CREDITS_CONFIG . " "
			. "(config_title, config_database, config_table, config_credits_col, config_creditsbonus_col, config_user_col, config_user_col_id, config_checkonline, config_display) "
			. "VALUES "
			. "(:title, :database, :table, :creditscol, :creditsbonuscol, :usercol, :usercolid, :checkonline, :display)";

		$saveConfig = $this->webdb->query($query, $data);
	...
	public function editConfig()
	{
		...
		$query = "UPDATE " . CABALENGINE_CREDITS_CONFIG . " SET "
			. "config_title = :title, "
			. "config_database = :database, "
			. "config_table = :table, "
			. "config_credits_col = :creditscol, "
			. "config_creditsbonus_col = :creditsbonuscol, "
		...
		$editConfig = $this->webdb->query($query, $data);
	...
	{
		if (!$this->_configId) throw new Exception(lang('error_66'));
		if (!$this->webdb->query("DELETE FROM " . CABALENGINE_CREDITS_CONFIG . " WHERE config_id = ?", array($this->_configId))) {
	...
	public function showConfigs($singleConfig = false)
	{
		if ($singleConfig) {
			if (!$this->_configId) throw new Exception(lang('error_66'));
			return $this->webdb->query_fetch_single("SELECT * FROM " . CABALENGINE_CREDITS_CONFIG . " WHERE config_id = ?", array($this->_configId));
		} else {
			$result = $this->webdb->query_fetch("SELECT * FROM " . CABALENGINE_CREDITS_CONFIG . " ORDER BY config_id ASC");
	 ...
	 */
	private function _addLog($configTitle = "unknown", $credits = 0, $creditsbonus, $transaction = "unknown")
	{
		...
		$query = "INSERT INTO " . CABALENGINE_CREDITS_LOGS . " "
			. "(log_config, log_identifier, log_credits, log_creditsbonus, log_transaction, log_date, log_inadmincp, log_module, log_ip) "
			. "VALUES "
			. "(:config, :identifier, :credits, :creditsbonus, :transaction, :timestamp, :inadmincp, :module, :ip)";

		$saveLog = $this->webdb->query($query, $data);
	}
	...
	{
		$query = str_replace(array('{LIMIT}'), array($limit), "SELECT TOP {LIMIT} * FROM " . CABALENGINE_CREDITS_LOGS . " ORDER BY log_id DESC");
		$result = $this->webdb->query_fetch($query);
	...
	public function getCreditsBonus()
	{
		...
		$variables = array('{TABLE}', '{COLUMN}', '{USER_COLUMN}');
		$values = array($config['config_table'], $config['config_creditsbonus_col'], $config['config_user_col']);
		$query = str_replace($variables, $values, "SELECT {COLUMN} FROM {TABLE} WHERE {USER_COLUMN} = :identifier");
		...
		return $getCredits[$config['config_creditsbonus_col']];
	}
}
