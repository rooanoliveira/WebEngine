...
class Account extends common
{
	...
	private $_defaultAccountSerial = '123456';
	...
	public function registerAccount($username, $password, $cpassword, $email)
	{
		...
		# query
		$query = "EXEC cabal_tool_registerAccount @id=:username, @password=:password, @email=:email, @name=:name, @userkey=:serial, @refercode=''";
	...
	{
		if (check_value($server)) {
			$result = $this->account->query_fetch("SELECT " . _CLMN_MS_MEMBID_ . ", " . _CLMN_MS_IP_ . " FROM " . _TBL_MS_ . " WHERE " . _CLMN_CONNSTAT_ . " = 1", array($server));
			if (!is_array($result)) return;
			return $result;
		}
		$result = $this->account->query_fetch("SELECT " . _CLMN_MS_MEMBID_ . ", " . _CLMN_MS_IP_ . " FROM " . _TBL_MS_ . " WHERE " . _CLMN_CONNSTAT_ . " = 1");
		if (!is_array($result)) return;
		return $result;
	}
