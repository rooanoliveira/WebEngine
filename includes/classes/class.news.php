...
class News
{
		...
		if (check_value($title) && check_value($content) && check_value($author)) {
			if ($this->checkTitle($title)) {
				if ($this->checkContent($content)) {
					...
					// add news
					$add_news = $this->db->query("INSERT INTO " . CABALENGINE_NEWS . " (news_title,news_author,news_date,news_content,allow_comments) VALUES (?,?,?,?,?)", $news_data);
	...
	function removeNews($id)
	{
		...
		if (Validator::Number($id)) {
			if ($this->newsIdExists($id)) {
				$remove = $this->db->query("DELETE FROM " . CABALENGINE_NEWS . " WHERE news_id = ?", array($id));
				if ($remove) {
	...
	function editNews($id, $title, $content, $author, $comments, $date)
	{
		...
		if (check_value($id) && check_value($title) && check_value($content) && check_value($author) && check_value($comments) && check_value($date)) {
			...
			if ($this->checkTitle($title) && $this->checkContent($content)) {
				...
				$query = $this->db->query("UPDATE " . CABALENGINE_NEWS . " SET news_title = ?, news_content = ?, news_author = ?, news_date = ?, allow_comments = ? WHERE news_id = ?", $editData);
				if ($query) {
	...
	{
		$this->db = Connection::Database('CabalEngine');
		$news = $this->db->query_fetch("SELECT * FROM " . CABALENGINE_NEWS . " ORDER BY news_id DESC");
		if (is_array($news)) {
	...
	{
		$this->db = Connection::Database('CabalEngine');
		$news = $this->db->query_fetch("SELECT news_id,news_title,news_author,news_date,allow_comments,news_content FROM " . CABALENGINE_NEWS . " ORDER BY news_id DESC");
		if (is_array($news)) {
			return $news;
		} else {
			return null;
	...
	function loadNewsData($id)
	{
		...
		if (check_value($id) && $this->newsIdExists($id)) {
			$query = $this->db->query_fetch_single("SELECT * FROM " . CABALENGINE_NEWS . " WHERE news_id = ?", array($id));
	...
	public function getNewsTranslations()
	{
		$this->db = Connection::Database('CabalEngine');
		if (!check_value($this->_id)) return;
		$newsTranslations = $this->db->query_fetch("SELECT * FROM " . CABALENGINE_NEWS_TRANSLATIONS . " WHERE news_id = ?", array($this->_id));
	...
	public function addNewsTransation()
	{
		...
		$result = $this->db->query("INSERT INTO " . CABALENGINE_NEWS_TRANSLATIONS . " (news_id, news_language, news_title, news_content) VALUES (?, ?, ?, ?)", array($this->_id, $this->_language, base64_encode($this->_title), base64_encode($this->_content)));
	...
	public function updateNewsTransation()
	{
		...
		$result = $this->db->query("UPDATE " . CABALENGINE_NEWS_TRANSLATIONS . " SET news_title = ?, news_content = ? WHERE news_id = ? AND news_language = ?", array(base64_encode($this->_title), base64_encode($this->_content), $this->_id, $this->_language));
	...
	public function deleteNewsTranslation()
	{
		...
		$result = $this->db->query("DELETE FROM " . CABALENGINE_NEWS_TRANSLATIONS . " WHERE news_id = ? AND news_language = ?", array($this->_id, $this->_language));
	...
	public function loadNewsTranslationData()
	{
		...
		$result = $this->db->query_fetch_single("SELECT * FROM " . CABALENGINE_NEWS_TRANSLATIONS . " WHERE news_id = ? AND news_language = ?", array($this->_id, $this->_language));
		if (!is_array($result)) return;
		return $result;
	}
	...
	public function getNewsTranslationsDataList()
	{
		$this->db = Connection::Database('CabalEngine');
		if (!check_value($this->_id)) return;
		$result = $this->db->query_fetch("SELECT * FROM " . CABALENGINE_NEWS_TRANSLATIONS . " WHERE news_id = ?", array($this->_id));
		if (!is_array($result)) return;
		return $result;
	}
