...
private function _guildsRanking() {
	...
	$query = "
		SELECT TOP ".$this->_results."
			g.GuildName,
			cc.Name AS GuildMaster,
			gl.Point AS GuildScore
		FROM Guild g
		INNER JOIN cabal_guild_level_table gl ON g.GuildNo = gl.GuildNo
		INNER JOIN GuildMember gm ON g.GuildNo = gm.GuildNo AND gm.GroupIndex = 1
		INNER JOIN cabal_character_table cc ON gm.CharacterIndex = cc.CharacterIdx
		WHERE g.GuildName NOT IN(".$this->_rankingsExcludeGuilds().")
		ORDER BY gl.Point DESC
	";
	...
	private function _getLevelRankingData($combineMasterLevel=false) {
		...
		if(!$combineMasterLevel) {
			$result = $this->server01->query_fetch("SELECT TOP ".$this->_results." "._CLMN_CHR_NAME_.","._CLMN_CHR_CLASS_.","._CLMN_CHR_LVL_.","._CLMN_CHR_MAP_." FROM "._TBL_CHR_." WHERE "._CLMN_CHR_NAME_." NOT IN(".$this->_rankingsExcludeChars().") ORDER BY "._CLMN_CHR_LVL_." DESC");
			if(!is_array($result)) return;
			return $result;
		}
	}
