...
class Vote {
	...
	public function vote() {
		...
		$creditSystem->addCredits($creditsReward, 0);
	...
	private function _canUserVote() {
		...
		$query = "SELECT * FROM ".CABALENGINE_VOTES." WHERE user_id = ? AND vote_site_id = ?";
		$check = $this->webdb->query_fetch_single($query, array($this->_userid, $this->_votesideId));
	...
	private function _canIPVote() {
		...
		$query = "SELECT * FROM ".CABALENGINE_VOTES." WHERE user_ip = ? AND vote_site_id = ?";
		$check = $this->webdb->query_fetch_single($query, array($this->_ip, $this->_votesideId));
	...
	private function _addRecord() {
		...
		$add = $this->webdb->query("INSERT INTO ".CABALENGINE_VOTES." (user_id, user_ip, vote_site_id, timestamp) VALUES (?, ?, ?, ?)", $data);
		if(!$add) throw new Exception(lang('error_23', true));
	}

	private function _removeRecord($id) {
		$remove = $this->webdb->query("DELETE FROM ".CABALENGINE_VOTES." WHERE id = ?", array($id));
	...
	private function _siteExists($id) {
		...
		$check = $this->webdb->query_fetch_single("SELECT * FROM ".CABALENGINE_VOTE_SITES." WHERE votesite_id = ?", array($id));
		if(is_array($check)) return true;
		return false;
	}
	...
	private function _logVote() {
		...
		$add_log = $this->webdb->query("INSERT INTO ".CABALENGINE_VOTE_LOGS." (user_id,votesite_id,timestamp) VALUES (?,?,?)", $add_data);
		if(!$add_log) return false;
		return true;
	}

	public function addVotesite($title, $link, $reward, $time) {
		$result = $this->webdb->query("INSERT INTO ".CABALENGINE_VOTE_SITES." (votesite_title,votesite_link,votesite_reward,votesite_time) VALUES (?,?,?,?)", array($title,$link,$reward,$time));
		if($result) return true;
	}

	public function deleteVotesite($id) {
		if(!$this->_siteExists($id)) return;
		$result = $this->webdb->query("DELETE FROM ".CABALENGINE_VOTE_SITES." WHERE votesite_id = ?", array($id));
		if($result) return $result;
	}

	public function retrieveVotesites($id=null) {
		if(check_value($id)) return $this->webdb->query_fetch_single("SELECT * FROM ".CABALENGINE_VOTE_SITES." WHERE votesite_id = ?", array($id));
		return $this->webdb->query_fetch("SELECT * FROM ".CABALENGINE_VOTE_SITES." ORDER BY votesite_id ASC");
	}
