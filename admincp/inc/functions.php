...
function checkVersion() {
	...
	$fields = array(
		'version' => urlencode(__CABALENGINE_VERSION__),
...
function getDownloadsList() {
	...
	$result = $db->query_fetch("SELECT * FROM ".CABALENGINE_DOWNLOADS." ORDER BY download_type ASC, download_id ASC");
...
function addDownload($title, $description='', $link, $size=0, $type=1) {
	...
	$result = $db->query("INSERT INTO ".CABALENGINE_DOWNLOADS." (download_title, download_description, download_link, download_size, download_type) VALUES (?, ?, ?, ?, ?)", array($title, $description, $link, $size, $type));
	if(!$result) return;
...
function editDownload($id, $title, $description='', $link, $size=0, $type=1) {
	...
	$result = $db->query("UPDATE ".CABALENGINE_DOWNLOADS." SET download_title = ?, download_description = ?, download_link = ?, download_size = ?, download_type = ? WHERE download_id = ?", array($title, $description, $link, $size, $type, $id));
	if(!$result) return;
...
function deleteDownload($id) {
	$db = Connection::Database('CabalEngine');
	if(!check_value($id)) return;
	$result = $db->query("DELETE FROM ".CABALENGINE_DOWNLOADS." WHERE download_id = ?", array($id));
	if(!$result) return;
...
function updateDownloadsCache() {
	$db = Connection::Database('CabalEngine');
	$downloadsData = $db->query_fetch("SELECT * FROM ".CABALENGINE_DOWNLOADS." ORDER BY download_type ASC, download_id ASC");
