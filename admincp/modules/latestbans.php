...
<?php
	...
	if(isset($_GET['liftban'])) {
		try {
			...
			$banInfo = $dbweb->query_fetch_single("SELECT * FROM ".CABALENGINE_BAN_LOG." WHERE id = ?", array($_GET['liftban']));
			...
			// Unban Account
			$unban = $dbaccount->query("UPDATE "._TBL_MI_." SET "._CLMN_BLOCCODE_." = 1 WHERE "._CLMN_USERNM_." = ?", array($banInfo['account_id']));
			if(!$unban) throw new Exception("Could not update account information (unban).");

			// Remove Ban log
			$dbweb->query("DELETE FROM ".CABALENGINE_BAN_LOG." WHERE account_id = ?", array($banInfo['account_id']));
			$dbweb->query("DELETE FROM ".CABALENGINE_BANS." WHERE account_id = ?", array($banInfo['account_id']));
	...
	<div class="col-md-12">
		<div class="panel-body">
			...
			<div class="tab-content">
				...
				<?php
					$tBans = $dbweb->query_fetch("SELECT TOP 25 * FROM ".CABALENGINE_BAN_LOG." WHERE ban_type = ? ORDER BY id DESC", array("temporal"));
					if(is_array($tBans)) {
				...
				<?php
					$pBans = $dbweb->query_fetch("SELECT TOP 25 * FROM ".CABALENGINE_BAN_LOG." WHERE ban_type = ? ORDER BY id DESC", array("permanent"));
					if(is_array($pBans)) {
						echo '<table class="table table-condensed">';
							echo '<thead>';
							echo '<tr>';
