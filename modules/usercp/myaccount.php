...
# account status
$accountStatus = ($accountInfo[_CLMN_BLOCCODE_] == 2 ? '<span class="label label-danger">'.lang('myaccount_txt_8').'</span>' : '<span class="label label-default">'.lang('myaccount_txt_7').'</span>');
		...
		foreach($AccountCharacters as $characterName) {
			...
			echo '<div class="col-xs-3">';
				...
				echo '<div class="myaccount-character-block">';
					...
					echo '</a>';
				echo '</div>';
				echo '<div class="myaccount-character-block-location">'.returnMapName($characterData[_CLMN_CHR_MAP_]).'</div>';
				echo '<span class="myaccount-character-block-level">'.$characterData[_CLMN_CHR_LVL_].'</span>';
				echo '</br>';
				echo '<span class="myaccount-character-block-level">'.getPlayerClass($characterData[_CLMN_CHR_CLASS_]).'</span>';
			echo '</div>';
		}
	echo '</div>';
} else {
...
if(defined('_TBL_CH_')) {
	...
	$this->db = Connection::Database('Account');
	$userNum = $this->common->retrieveUserID($_SESSION['username']);
		if(!$userNum) return;

	$connectionHistory = $db->query_fetch("SELECT TOP 10 * FROM "._TBL_CH_." WHERE "._CLMN_CH_ACCID_." =  ORDER BY "._CLMN_CH_DATE_." DESC", array($userNum));
		...
		echo '<table class="table table-condensed general-table-ui">';
			...
			foreach($connectionHistory as $row) {
				echo '<tr>';
					...
					echo '<td>'.$row[_CLMN_CH_DATEOUT_].'</td>';
